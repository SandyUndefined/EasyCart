...
<script type="text/javascript">
        ...
        ref.get().then(async function (snapshots) {
            ...
            $.fn.dataTable.ext.search.push(
                function(settings, data, dataIndex) {
                    var searchTerm = $('#orderTable_filter input').val().trim().toLowerCase();
                    var statusText = $('<div>').html(data[8]).text().trim().toLowerCase(); // Extract the text content from HTML

                    if (searchTerm.toLowerCase() == "order rejected" || searchTerm.toLowerCase() == "driver rejected") {
                        return statusText === searchTerm; // Match exact text
                    }
                    return true; // Include all rows if search term is empty
                }
            );
            if (getId != '' || driverID || userID && checkDeletePermission) {
                ...
                $('#orderTable').DataTable({
                    ...
                    columnDefs: [
                        ...
                        {
                            targets: 8,  // Order status column
                            render: function(data, type, row, meta) {
                                if (type === 'display') {
                                    return data; // Display the original HTML
                                }
                                return $('<div>').html(data).text(); // Use only the text content for sorting and searching
                            }
                        },
            ...
            $('#orderTable_filter input').on('keyup change', function() {
    ...
    async function getListData(val) {
        ...
        html = html + '<td><a href="' + route1 + '" class="redirecttopage">' + val.id + '</a></td>';
        ...
        if (checkDeletePermission) {
           html = html + '<a id="' + val.id + '" class="do_not_delete" name="order-delete" href="javascript:void(0)"><i class="fa fa-trash"></i></a></td>';
        }
