...
class _AddOrUpdateProductScreenState extends State<AddOrUpdateProductScreen> {
                    ...
                    Expanded(
                      child: SingleChildScrollView(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.stretch,
                          children: [
                            adminCommissionModel == null || adminCommissionModel!.isEnabled == false
                                ? SizedBox()
                                : Column(
                                    crossAxisAlignment: CrossAxisAlignment.stretch,
                                    children: [
                                      Container(
                                        decoration: BoxDecoration(color: Colors.red.withOpacity(0.20), borderRadius: BorderRadius.all(Radius.circular(10))),
                                        child: Padding(
                                          padding: const EdgeInsets.all(8.0),
                                          child: Text(
                                            "The price for product includes the admin commission. For instance, if the product is priced at \$100 and the admin commission is 10% , the effective price would be \$110 for customer, which will apply automatically.",
                                      ...
                                      Container(
                                        ...
                                        child: Padding(
                                          padding: const EdgeInsets.all(8.0),
                                          child: Text(
                                            "Admin Commission : ${commissionType()}",
                                            style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: 16),
                                          ),
                                        ),
                                      ),
                                ...
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        amountShow(amount: rprice.text.isEmpty ? "0" : rprice.text),
                                ...
                                Flexible(
                                  ...
                                  children: [
                                    SwitchListTile.adaptive(
                                        activeColor: Color(COLOR_ACCENT),
                                        title:
                                            Text('Non-veg'.tr(), style: TextStyle(fontSize: 16, color: isDarkMode(context) ? Colors.white : Colors.black, fontFamily: "Poppins")),
                                        value: nonVeg,
                                        onChanged: (bool newValue) async {
                                          nonVeg = newValue;
                                          if (nonVeg == true) {
                            ...
                            SwitchListTile.adaptive(
                                activeColor: Color(COLOR_ACCENT),
                                title:
                                    Text('Takeaway Option'.tr(), style: TextStyle(fontSize: 16, color: isDarkMode(context) ? Colors.white : Colors.black, fontFamily: "Poppinsl")),
                                value: takeaway,
                                onChanged: (bool newValue) async {
                                  setState(() {
                                    takeaway = newValue;
  ...
  submit() async {
    if (veg == false && nonVeg == false) {
    ...
    } else if (double.parse(rprice.text.toString()) <= 0) {
      showimgAlertDialog(context, 'Valid amount is Required!'.tr(), 'Please enter valid regular price'.tr(), true);
    } else {
      if (_key.currentState?.validate() ?? false) {
        ...
        await showProgress(widget.product == null ? 'Adding product...'.tr() : 'Applying edits...'.tr(), false);
      ...
      actions: <Widget>[
        CupertinoActionSheetAction(
          ...
          onPressed: () async {
            ...
            if (singleImage != null) {
              await showProgress('Image Upload...'.tr(), false);
