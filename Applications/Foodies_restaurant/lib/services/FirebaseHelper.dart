...
import 'package:foodie_restaurant/model/document_model.dart';
import 'package:foodie_restaurant/model/driver_document_model.dart';
      ...
      if (userDocument.data() != null && userDocument.exists) {
        try {
          ...
          await firestore
              .collection(USERS)
              .doc(userId)
              .update({"wallet_amount": (num.parse(userDocument.data()!['wallet_amount'].toString()) + amount)}).then((value) => print("north"));
        } catch (error) {
          print(error);
      ...
      if (MyAppState.currentUser!.vendorID.isNotEmpty) {
        await firestore.collection(COUPONS).where('resturant_id', isEqualTo: MyAppState.currentUser!.vendorID).get().then((value) async {
          for (var doc in value.docs) {
            await firestore.collection(COUPONS).doc(doc.reference.id).delete();
          }
        });
        await firestore.collection(FOOD_REVIEW).where('VendorId', isEqualTo: MyAppState.currentUser!.vendorID).get().then((value) async {
          for (var doc in value.docs) {
            await firestore.collection(FOOD_REVIEW).doc(doc.reference.id).delete();
          }
        });
        ...
        await firestore.collection(PRODUCTS).where('vendorID', isEqualTo: MyAppState.currentUser!.vendorID).get().then((value) async {
          for (var doc in value.docs) {
            ...
            await firestore.collection(PRODUCTS).doc(doc.reference.id).delete();
          }
        });
        await firestore.collection(VENDORS).doc(MyAppState.currentUser!.vendorID).delete();
      }

      await firestore.collection(USERS).doc(MyAppState.currentUser!.userID).delete();
