...
import 'package:foodie_driver/model/zone_model.dart';
...
class FireStoreUtils {
  ...
  Future<Url> uploadAudioFile(File file, BuildContext context) async {
    await showProgress(context, 'Uploading Audio...', false);
    var uniqueID = Uuid().v4();
  ...
  static firebaseSignUpWithEmailAndPassword(
      String emailAddress, String password, File? image, File? carImage, String firstName, String lastName, String mobile,String zoneId) async {
    try {
  ...
  static Future<List<DocumentModel>> getDocumentList() async {
    ...
    await firestore.collection(documents).where('type',isEqualTo: "driver").where('enable', isEqualTo: true).get().then((value) {
      for (var element in value.docs) {
